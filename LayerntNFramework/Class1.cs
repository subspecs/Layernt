    ...
    public static class Layernt
    {
        ...
        public static void EncryptImage24(string InputImage, string OutputImage, byte[] Buffer, int Offset, int Count, int SaveBits, byte[] EncryptionPassword, string DataFileName = null, bool UsePlatformSafeEncryption = false)
        {
            ...
            unsafe
            {
                ...
                IReadOnlyList<System.Memory<Rgb24>> MemoryBang = Image.GetPixelMemoryGroup(); var BufferHandles = new System.Buffers.MemoryHandle[MemoryBang.Count]; Pixel24** RawPixelArray = (Pixel24**)System.Runtime.InteropServices.Marshal.AllocHGlobal(sizeof(void*) * MemoryBang.Count);
                int e = 0, BytesPerBang = MemoryBang[0].Length; while (e < MemoryBang.Count) { BufferHandles[e] = MemoryBang[e].Pin(); RawPixelArray[e] = (Pixel24*)BufferHandles[e].Pointer; e++; }
                Pixel24* GetPixel(Pixel24** PXArray, int BPB, int Offset2) { return &PXArray[Offset2 / BPB][Offset2 - ((Offset2 / BPB) * BPB)]; }
                ...
                System.Security.Cryptography.Aes AES = new System.Security.Cryptography.AesGcm(PWHash);
                ...
                AES.Dispose();
                n = 0; while (n < BufferHandles.Length) { BufferHandles[n].Dispose(); n++; }; System.Runtime.InteropServices.Marshal.FreeHGlobal((System.IntPtr)RawPixelArray);
                Image.Dispose();
            }
        ...
        public static void DecryptImage24(string InputImage, out byte[] Output, byte[] EncryptionPassword, out string DataFileName)
        {
            ...
            unsafe
            {
                ...
                IReadOnlyList<System.Memory<Rgb24>> MemoryBang = Image.GetPixelMemoryGroup(); var BufferHandles = new System.Buffers.MemoryHandle[MemoryBang.Count]; Pixel24** RawPixelArray = (Pixel24**)System.Runtime.InteropServices.Marshal.AllocHGlobal(sizeof(void*) * MemoryBang.Count);
                int e = 0, BytesPerBang = MemoryBang[0].Length; while (e < MemoryBang.Count) { BufferHandles[e] = MemoryBang[e].Pin(); RawPixelArray[e] = (Pixel24*)BufferHandles[e].Pointer; e++; }
                Pixel24* GetPixel(Pixel24** PXArray, int BPB, int Offset) { return &PXArray[Offset / BPB][Offset - ((Offset / BPB) * BPB)]; }
                ...
                void ReadInPixels(int TotalPixelCount, byte[] Buffer, Pixel24** PXArray, int BPB, int SaveBitsx)
                {
                    int nx = 1, z; long BitOffset = 0, BufferTotalBits = Buffer.Length * 8; while (nx < TotalPixelCount && BitOffset != BufferTotalBits) //+1 for the first save bits pixel.
                    {
                        z = 0; while (z < SaveBitsx && BitOffset != BufferTotalBits) { Bits.SetBit(ref Buffer[BitOffset / 8], (int)(BitOffset % 8), Bits.GetBit((*GetPixel(PXArray, BPB, nx)).B, z)); z++; BitOffset++; }
                        z = 0; while (z < SaveBitsx && BitOffset != BufferTotalBits) { Bits.SetBit(ref Buffer[BitOffset / 8], (int)(BitOffset % 8), Bits.GetBit((*GetPixel(PXArray, BPB, nx)).G, z)); z++; BitOffset++; }
                        z = 0; while (z < SaveBitsx && BitOffset != BufferTotalBits) { Bits.SetBit(ref Buffer[BitOffset / 8], (int)(BitOffset % 8), Bits.GetBit((*GetPixel(PXArray, BPB, nx)).R, z)); z++; BitOffset++; }
                        nx++;
                    }
                }
                ...
                AES.Dispose();
                int n = 0; while (n < BufferHandles.Length) { BufferHandles[n].Dispose(); n++; }; System.Runtime.InteropServices.Marshal.FreeHGlobal((System.IntPtr)RawPixelArray);
                Image.Dispose();
            }
        }
        ...
        public static void EncryptImage32(string InputImage, string OutputImage, byte[] Buffer, int Offset, int Count, int SaveBits, byte[] EncryptionPassword, string DataFileName = null, bool UsePlatformSafeEncryption = false)
        {
            ...
            unsafe
            {
                ...
                IReadOnlyList<System.Memory<Rgba32>> MemoryBang = Image.GetPixelMemoryGroup(); var BufferHandles = new System.Buffers.MemoryHandle[MemoryBang.Count]; Pixel32** RawPixelArray = (Pixel32**)System.Runtime.InteropServices.Marshal.AllocHGlobal(sizeof(void*) * MemoryBang.Count);
                int e = 0, BytesPerBang = MemoryBang[0].Length; while (e < MemoryBang.Count) { BufferHandles[e] = MemoryBang[e].Pin(); RawPixelArray[e] = (Pixel32*)BufferHandles[e].Pointer; e++; }
                Pixel32* GetPixel(Pixel32** PXArray, int BPB, int Offset2) { return &PXArray[Offset2 / BPB][Offset2 - ((Offset2 / BPB) * BPB)]; }
        ...
        public static void DecryptImage32(string InputImage, out byte[] Output, byte[] EncryptionPassword, out string DataFileName)
        {
            ...
            unsafe
            {
                ...
                IReadOnlyList<System.Memory<Rgba32>> MemoryBang = Image.GetPixelMemoryGroup(); var BufferHandles = new System.Buffers.MemoryHandle[MemoryBang.Count]; Pixel32** RawPixelArray = (Pixel32**)System.Runtime.InteropServices.Marshal.AllocHGlobal(sizeof(void*) * MemoryBang.Count);
                int e = 0, BytesPerBang = MemoryBang[0].Length; while (e < MemoryBang.Count) { BufferHandles[e] = MemoryBang[e].Pin(); RawPixelArray[e] = (Pixel32*)BufferHandles[e].Pointer; e++; }
                Pixel32* GetPixel(Pixel32** PXArray, int BPB, int Offset) { return &PXArray[Offset / BPB][Offset - ((Offset / BPB) * BPB)]; }
                ...
                void ReadInPixels(int TotalPixelCount, byte[] Buffer, Pixel32** PXArray, int BPB, int SaveBitsx)
                {
                    int nx = 1, z; long BitOffset = 0, BufferTotalBits = Buffer.Length * 8; while (nx < TotalPixelCount && BitOffset != BufferTotalBits) //+1 for the first save bits pixel.
                    {
                        z = 0; while (z < SaveBitsx && BitOffset != BufferTotalBits) { Bits.SetBit(ref Buffer[BitOffset / 8], (int)(BitOffset % 8), Bits.GetBit((*GetPixel(PXArray, BPB, nx)).B, z)); z++; BitOffset++; }
                        z = 0; while (z < SaveBitsx && BitOffset != BufferTotalBits) { Bits.SetBit(ref Buffer[BitOffset / 8], (int)(BitOffset % 8), Bits.GetBit((*GetPixel(PXArray, BPB, nx)).G, z)); z++; BitOffset++; }
                        z = 0; while (z < SaveBitsx && BitOffset != BufferTotalBits) { Bits.SetBit(ref Buffer[BitOffset / 8], (int)(BitOffset % 8), Bits.GetBit((*GetPixel(PXArray, BPB, nx)).R, z)); z++; BitOffset++; }
                        z = 0; while (z < SaveBitsx && BitOffset != BufferTotalBits) { Bits.SetBit(ref Buffer[BitOffset / 8], (int)(BitOffset % 8), Bits.GetBit((*GetPixel(PXArray, BPB, nx)).A, z)); z++; BitOffset++; }
                        nx++;
                    }
                }
        ...
        public static void EncryptImage48(string InputImage, string OutputImage, byte[] Buffer, int Offset, int Count, int SaveBits, byte[] EncryptionPassword, string DataFileName = null, bool UsePlatformSafeEncryption = false)
        {
            ...
            unsafe
            {
                ...
                IReadOnlyList<System.Memory<Rgb48>> MemoryBang = Image.GetPixelMemoryGroup(); var BufferHandles = new System.Buffers.MemoryHandle[MemoryBang.Count]; Pixel48** RawPixelArray = (Pixel48**)System.Runtime.InteropServices.Marshal.AllocHGlobal(sizeof(void*) * MemoryBang.Count);
                int e = 0, BytesPerBang = MemoryBang[0].Length; while (e < MemoryBang.Count) { BufferHandles[e] = MemoryBang[e].Pin(); RawPixelArray[e] = (Pixel48*)BufferHandles[e].Pointer; e++; }
                Pixel48* GetPixel(Pixel48** PXArray, int BPB, int Offset2) { return &PXArray[Offset2 / BPB][Offset2 - ((Offset2 / BPB) * BPB)]; }
        ...
        public static void DecryptImage48(string InputImage, out byte[] Output, byte[] EncryptionPassword, out string DataFileName)
        {
            ...
            unsafe
            {
                ...
                IReadOnlyList<System.Memory<Rgb48>> MemoryBang = Image.GetPixelMemoryGroup(); var BufferHandles = new System.Buffers.MemoryHandle[MemoryBang.Count]; Pixel48** RawPixelArray = (Pixel48**)System.Runtime.InteropServices.Marshal.AllocHGlobal(sizeof(void*) * MemoryBang.Count);
                int e = 0, BytesPerBang = MemoryBang[0].Length; while (e < MemoryBang.Count) { BufferHandles[e] = MemoryBang[e].Pin(); RawPixelArray[e] = (Pixel48*)BufferHandles[e].Pointer; e++; }
                Pixel48* GetPixel(Pixel48** PXArray, int BPB, int Offset) { return &PXArray[Offset / BPB][Offset - ((Offset / BPB) * BPB)]; }
                ...
                void ReadInPixels(int TotalPixelCount, byte[] Buffer, Pixel48** PXArray, int BPB, int SaveBitsx)
                {
                    int nx = 1, z; long BitOffset = 0, BufferTotalBits = Buffer.Length * 8; while (nx < TotalPixelCount && BitOffset != BufferTotalBits) //+1 for the first save bits pixel.
                    {
                        z = 0; while (z < SaveBitsx && BitOffset != BufferTotalBits) { Bits.SetBit(ref Buffer[BitOffset / 8], (int)(BitOffset % 8), Bits.GetBit((*GetPixel(PXArray, BPB, nx)).B, z)); z++; BitOffset++; }
        ...
        public static void EncryptImage64(string InputImage, string OutputImage, byte[] Buffer, int Offset, int Count, int SaveBits, byte[] EncryptionPassword, string DataFileName = null, bool UsePlatformSafeEncryption = false)
        {
            ...
            unsafe
            {
                ...
                IReadOnlyList<System.Memory<Rgba64>> MemoryBang = Image.GetPixelMemoryGroup(); var BufferHandles = new System.Buffers.MemoryHandle[MemoryBang.Count]; Pixel64** RawPixelArray = (Pixel64**)System.Runtime.InteropServices.Marshal.AllocHGlobal(sizeof(void*) * MemoryBang.Count);
                int e = 0, BytesPerBang = MemoryBang[0].Length; while (e < MemoryBang.Count) { BufferHandles[e] = MemoryBang[e].Pin(); RawPixelArray[e] = (Pixel64*)BufferHandles[e].Pointer; e++; }
                Pixel64* GetPixel(Pixel64** PXArray, int BPB, int Offset2) { return &PXArray[Offset2 / BPB][Offset2 - ((Offset2 / BPB) * BPB)]; }
        ...
        public static void DecryptImage64(string InputImage, out byte[] Output, byte[] EncryptionPassword, out string DataFileName)
        {
            ...
            unsafe
            {
                ...
                IReadOnlyList<System.Memory<Rgba64>> MemoryBang = Image.GetPixelMemoryGroup(); var BufferHandles = new System.Buffers.MemoryHandle[MemoryBang.Count]; Pixel64** RawPixelArray = (Pixel64**)System.Runtime.InteropServices.Marshal.AllocHGlobal(sizeof(void*) * MemoryBang.Count);
                int e = 0, BytesPerBang = MemoryBang[0].Length; while (e < MemoryBang.Count) { BufferHandles[e] = MemoryBang[e].Pin(); RawPixelArray[e] = (Pixel64*)BufferHandles[e].Pointer; e++; }
                Pixel64* GetPixel(Pixel64** PXArray, int BPB, int Offset) { return &PXArray[Offset / BPB][Offset - ((Offset / BPB) * BPB)]; }
                ...
                void ReadInPixels(int TotalPixelCount, byte[] Buffer, Pixel64** PXArray, int BPB, int SaveBitsx)
                {
                    int nx = 1, z; long BitOffset = 0, BufferTotalBits = Buffer.Length * 8; while (nx < TotalPixelCount && BitOffset != BufferTotalBits) //+1 for the first save bits pixel.
                    {
                        z = 0; while (z < SaveBitsx && BitOffset != BufferTotalBits) { Bits.SetBit(ref Buffer[BitOffset / 8], (int)(BitOffset % 8), Bits.GetBit((*GetPixel(PXArray, BPB, nx)).B, z)); z++; BitOffset++; }
